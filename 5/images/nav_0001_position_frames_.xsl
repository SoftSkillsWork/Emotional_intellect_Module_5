...
<xsl:template match="params">
    ...
    <style>
        .cl-position-table-frame { width: <xsl:value-of select="$width"/>px; height: <xsl:value-of select="$height"/>px; }
        .cl-position-table-frame td { width: <xsl:value-of select="$width"/>px; height: <xsl:value-of select="$height"/>px; }
        .cl-position-frame { color: <xsl:value-of select="$font-color-fixed"/>; font-size: <xsl:value-of select="font_size"/>px; font-family: <xsl:value-of select="$_font"/>;<xsl:value-of select="$_css_font_style"/><xsl:if test="text_shadow_strength!='none'">text-shadow: <xsl:value-of select="$_text_shadow"/>;</xsl:if> }
    </style>
    <div>
        <xsl:attribute name="class">clo-nav_0001_position-frames</xsl:attribute>
        <div>
            <xsl:attribute name="class">cl-container-frame</xsl:attribute>
            <table>
                <xsl:attribute name="class">cl-position-table-frame</xsl:attribute>
                <tr>
                    <td>
                        <div class="position">
                            <xsl:attribute name="class">cl-position-frame</xsl:attribute>
                            <span class="cl-label-frame"><xsl:value-of select="position_text"/></span>
                            <span class="cl-value-frame">1</span>
                            <span class="cl-divider-frame"><xsl:value-of select="position_divider"/></span>
                            <span class="cl-total-frame">1</span>
                        </div>
                    </td>
                </tr>
            </table>
